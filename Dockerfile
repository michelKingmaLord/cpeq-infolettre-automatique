...
ENV VIRTUAL_ENV /opt/cpeq-infolettre-automatique-env
ENV PATH $VIRTUAL_ENV/bin:$PATH
RUN python -m venv $VIRTUAL_ENV

# Set the working directory.
WORKDIR /workspaces/cpeq-infolettre-automatique/
...
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/cpeq-infolettre-automatique/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/cpeq_infolettre_automatique/ && touch src/cpeq_infolettre_automatique/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/cpeq-infolettre-automatique/
...
ENTRYPOINT ["/opt/cpeq-infolettre-automatique-env/bin/poe"]
CMD ["api"]
